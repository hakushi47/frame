...
export async function listTemplates(): Promise<KataTemplate[]> {
  const templates = await readJsonFile<KataTemplate>(TEMPLATES_FILE);
  return templates.sort((a, b) => (a.createdAt < b.createdAt ? 1 : -1));
}

export async function getTemplate(id: string): Promise<KataTemplate | undefined> {
  const templates = await readJsonFile<KataTemplate>(TEMPLATES_FILE);
  return templates.find((template) => template.id === id);
}

export async function addTemplate(template: KataTemplate): Promise<void> {
  const templates = await readJsonFile<KataTemplate>(TEMPLATES_FILE);
...
export async function toggleFavorite(shotId: string): Promise<void> {
  const shots = await readJsonFile<Shot>(SHOTS_FILE);
  const updated = shots.map((shot) =>
    shot.id === shotId ? { ...shot, favorite: !shot.favorite } : shot
  );
  await writeJsonFile(SHOTS_FILE, updated);
}
